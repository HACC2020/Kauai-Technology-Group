...
app.get('/v1/items', async (req, res) => {
  const items = await pgClient.query('SELECT * FROM density');
  res.status(200).send(items.rows);
});

// Get a single todo item
app.get('/v1/item', async (req, res) => {
  var start_date = new Date(parseInt(req.query.start_date)).toISOString();
  var end_date = new Date(parseInt(req.query.start_date)+10800000).toISOString();
  const items = await pgClient
    .query('SELECT * FROM density WHERE date_start >= $1 AND date_start < $2',
            [start_date, end_date])
    .catch(e => {
      res
        .status(500)
